...
void main() {
	...
	mkdirRecurse("out/assets/minecraft/optifine/cit/fishcolors");
    ...
    foreach (specificValue; specificValues) {
        uint primary = (specificValue >> 16) & 0xFF;
        uint secondary = (specificValue >> 24) & 0xFF;

        fishValue = specificValue;

        writeColoredImage(img, "out/assets/minecraft/optifine/cit/fishcolors/%s.png".format(fishValue), COLORS[primary], COLORS[secondary]);
        std.file.write("out/assets/minecraft/optifine/cit/fishcolors/%s.properties".format(fishValue), import("stubModel.properties").format(specificValue, specificValue));
    }
}
